      ...
      <View style={styles.overlay}>
        <View style={styles.drawer}>
          ...
          <TouchableOpacity style={styles.closeButton} onPress={onClose}>
            <Ionicons name="close" size={28} color={colors.light.mutedForeground} />
          </TouchableOpacity>
          ...
          <View style={styles.plansContainer}>
            <PlanButton label="CHF 4.00 Weekly" onPress={() => onPurchase('weekly')} />
            <PlanButton label="CHF 6.00 Monthly access" onPress={() => onPurchase('monthly')} />
            <PlanButton
          ...
          <Text style={styles.infoText}>
            Recurring billing. You will be charged to your App Store account. You can cancel your subscription anytime. Read more in the <Text style={styles.link} onPress={onPrivacyPolicy}>Terms of usage</Text>
          </Text>
          ...
          {/* Unlock button */}
          <TouchableOpacity style={styles.unlockBtn} onPress={() => onPurchase('yearly')}>
...
const FeatureItem = ({ text }: { text: string }) => (
  <View style={styles.featureItem}>
    <Ionicons name="checkmark-circle" size={20} color={colors.light.primary} style={{ marginRight: 8 }} />
    <Text style={styles.featureText}>{text}</Text>
  </View>
);

const PlanButton = ({ label, highlight, badge, onPress }: { label: string; highlight?: boolean; badge?: string; onPress: () => void }) => (
  <TouchableOpacity
    style={[styles.planBtn, highlight && styles.planBtnHighlight]}
    onPress={onPress}
    activeOpacity={0.85}
  >
    <Text style={[styles.planBtnText, highlight && styles.planBtnTextHighlight]}>{label}</Text>
    {badge && (
